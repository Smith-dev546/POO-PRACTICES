...
abstract class Pago{
    ...
    //Metodo abtracto: Obliga a las hijas a definir como procesan el dinero
...
rl.question("Seleccione metodo (1: Efectivo, 2: Tarjeta): ", (opcion:string)=>{
    rl.question('Ingrese el monto a pagar: ',(montoInput: string)=>{
        const montoNum = parseFloat(montoInput);
        let miPago: Pago;   //Vaariable de tipo Padre (Polimorfismo)

        if(opcion === "1"){
            miPago = new PagoEfectivo(montoNum);
            FinalizarTransaccion(miPago);
        }else if(opcion === "2"){
            rl.question("Ingrese su numero de tarjeta: ",(tarjeta:string)=>{
